...
@RestController @RequestMapping("/api/v1/imports") @org.springframework.boot.autoconfigure.condition.ConditionalOnProperty(name="app.imports.enabled",havingValue="true",matchIfMissing=true) public class ImportController {
 private final ImportService service; public ImportController(ImportService service){this.service=service;}
  @PostMapping(consumes=MediaType.MULTIPART_FORM_DATA_VALUE) public ResponseEntity<ImportResponse> create(@RequestParam MultipartFile file,@RequestParam(defaultValue="generic") String profile,@RequestParam(required=false) UUID accountId,CurrentUser currentUser)throws Exception{return ResponseEntity.status(HttpStatus.CREATED).body(service.create(ProviderInput.file(file.getOriginalFilename(),profile,Instant.now(),file.getBytes()).withAccount(accountId),currentUser.id()));}
  @PostMapping(consumes=MediaType.APPLICATION_JSON_VALUE) public ResponseEntity<ImportResponse> createManual(@RequestBody ManualRequest request,CurrentUser currentUser){return ResponseEntity.status(HttpStatus.CREATED).body(service.create(ProviderInput.manual("manual",Instant.now(),request.entries()).withAccount(request.accountId()),currentUser.id()));}
  @GetMapping("/{id}/preview") public ImportResponse preview(@PathVariable UUID id,CurrentUser currentUser){return service.preview(id,currentUser.id());}
  @PostMapping("/{id}/commit") public ImportResponse commit(@PathVariable UUID id,@RequestBody CommitRequest request,CurrentUser currentUser){return service.commit(id,currentUser.id(),request.confirmationToken(),request.version());}
  @DeleteMapping("/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) public void discard(@PathVariable UUID id,CurrentUser currentUser){service.discard(id,currentUser.id());}
